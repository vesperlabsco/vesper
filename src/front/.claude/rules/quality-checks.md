...
- Règles actives : `typescript-eslint` en mode `strictTypeChecked` (type-aware, via `projectService`), `eslint-plugin-react` (+ `jsx-runtime`), `eslint-plugin-jsx-a11y`, `eslint-plugin-import-x` (+ résolveur TypeScript), `eslint-plugin-react-hooks`, `eslint-plugin-react-refresh`, `eslint-plugin-i18next` (`no-literal-string` — fait respecter la règle "jamais de texte en dur" de [[i18n.md]] au niveau du linter, pas seulement à la revue), `eslint-plugin-jsonc` (hygiène des fichiers `.json`, ex. `no-dupe-keys` — `jsonc/no-comments` désactivé spécifiquement sur `tsconfig*.json` qui est du JSONC), `eslint-config-prettier` en dernier pour désactiver tout ce qui recoupe le formatter.
