...
> **[À METTRE À JOUR]** Aucune stack de test unitaire n'est installée à ce jour (pas de vitest/jest dans `package.json`, pas de script `test`). `make front_test` existe côté Makefile mais échoue tant que le script npm correspondant n'existe pas. Une fois la stack choisie et installée, remplacer cette note par la commande réelle et retirer ce bandeau.
